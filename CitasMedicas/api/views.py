...
from api.generate_code import get_code  # Si lo tienes
import api.send_message as send_message  # Si lo tienes

# Función para obtener los servicios
def get_service_object(index=None):
    service = Servicios.objects.values_list('id', 'name')
    if index is None:
        return service
    else:
...
# Función para obtener los médicos que ofrecen un servicio
def get_doctor_services_object(service_id, index=None):
    doctor_service = ServiciosMedico.objects.filter(
...
def get_doctor_time_slot_object(doctor_service_id, index=None):
    doctor_time_slots = HorariosMedico.objects.filter(
...
# Función para generar la lista de médicos para un servicio
...
# Función para generar la lista de horarios disponibles de un médico
...
# Vista USSD Callback para manejar la lógica
@api_view(['GET', 'POST'])
def ussd_callback(request):
    ...
    if text == '':
        ...
        if len(values) == 1:
            ...
            doctor_time_slot = HorariosMedico.objects.get(pk=selected_doctor_time_slot_id)
            patient = Patients.objects.filter(phone_number=phone_number)
            booking_code = get_code()

            if len(patient) > 0:
                appointment = Citas.objects.create(doctor_time_slots=doctor_time_slot, patient=patient.first(), booking_code=booking_code)
